...
def main() -> None:
    # W10 zelf blijft de gezaghebbende Step-4 fixture zonder urenpositie.
    # Voor het geïntegreerde dashboard heeft dezelfde persoon aanvullend een
    # volledige Sportlink A/B/C/D-positie nodig om Step 5 te kunnen uitvoeren.
    dashboard_away_case = replace(
        W_CASE_BY_ID["W10"],
        case_id="DASH-AWAY",
    ...
    cases = (W_CASE_BY_ID["W08"], W_CASE_BY_ID["W07"], dashboard_away_case)
    services = (
    ...
    priorities = tuple(
        ...
        for priority in prioritize_candidates(
            tuple(a for a in assessments if a.service_id == service.service_id),
            cases,
            service.starts_at.date(),
        )
    )
    dashboard = build_dashboard(
        cases, decisions, services, teams, assessments, priorities,
    ...
    for row in dashboard.duty_rows:
        ...
        print(
            f"      Verplicht {row.required_hours}; uitgevoerd {row.completed_hours}; "
            f"al ingepland {row.scheduled_hours} uur."
        )
    ...
    for row in dashboard.service_rows:
        ...
        print(
            f"   {row.service_type.capitalize()} — {row.starts_at:%d-%m-%Y} "
            f"{row.starts_at:%H:%M}-{row.ends_at:%H:%M}: nog {row.remaining_staff} {plek} te bezetten."
        )
    ...
    for service in dashboard.service_rows:
        ...
        for row in advised:
            ...
            print(
                f"   {label}: {row.name} uit {row.team_id} — nog {row.remaining_hours} uur; "
                f"{_wedstrijdtekst(row)}."
            )
