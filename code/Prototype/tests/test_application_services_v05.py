...
def test_import_application_service_previews_then_confirms_atomically(tmp_path):
    ...
    with db.unit_of_work() as uow:
        ...
        uow.commit()
    with db.unit_of_work() as uow:
        app = ImportApplicationService(uow)
        ...
        snapshot = app.confirm(batch, records, snapshot_id="S1", confirmed_at=NOW, confirmed_by="planner")
...
def test_engine_run_application_service_records_versioned_run(tmp_path):
    ...
    with db.unit_of_work() as uow:
        ...
        app = EngineRunApplicationService(uow)
